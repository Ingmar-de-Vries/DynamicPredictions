function DynamicPredictions_ERFdynamicRSA_searchlight(cfg,iSub,~)
...
omegaPerIter = zeros(cfg.randshuff(1),length(atlas),size(cfg.peaks2test,1));
for iter = 1:cfg.randshuff(1)
    ...
    % zeros, which in turn results in NaNs when performing correlation for RDMs (but only at the edges that are chopped off below anyway)
    ...
    % because outside those bounds some RDM vectors are not defined (i.e., they contain extrapolated values)
...
%         omega = zeros(newtimelength,size(cfg.peaks2test,1),length(atlas));
%         for iRDM = 1:length(cfg.models2test)% only test for our standard 7
%             
%             % neural - model correlation
%             omega(:,:,iRDM) = corr(squeeze(modelRDM(cfg.models2test(iRDM),:,:))',squeeze(neuralRDM(:,:,isource)));
%             
%         end
        ...
        for ipeak = 1:size(cfg.peaks2test,1)% peaks from dRSA curves to run the searchlight on
            for ibin1 = 1:size(modelRDM,2)% model time
                ...
                if lag2test<1 || lag2test>newtimelength
                    omega(ibin1,ipeak,:) = nan(length(atlas),1);% this lag is not defined at the edge of the stimulus window
                else
                    X = [Xtest Xregressout];
    ...
    omega = squeeze(nanmean(omega));% average over stimulus time. On edges there are some NaNs because some lags fall outside of resampled stimulus window. 
    
    % combine data from all iterations
    omegaPerIter(iter,:,:) = omega';
    clear omega
